...
/*$mainForm[0].querySelector('[data-ak="first-time-visitor"]').addEventListener('change', e => {
...
$mainForm[0].querySelector('input[type=submit]').addEventListener('click', e => {
  ...
  /*const validatedInputs = validateInputFields();
  if (validatedInputs) {
    let checkoutUrl = 'https://askkhonsu.foxycart.com/cart?cart=checkout&name=Tailored%20Activites&price=49.99&category=plan';

    if (!localStorage['ak-flatpickrDateObj']) return;
    const { selectedDates, dateStr } = JSON.parse(localStorage['ak-flatpickrDateObj']);
    
    checkoutUrl = updatePricingOnCheckoutUrl(dateStr, checkoutUrl);
    checkoutUrl = updateTravelDatesOnCheckoutUrl(selectedDates, checkoutUrl);
...
/*function updatePricingOnCheckoutUrl(dateStr, checkoutUrl) {
  const price = getPricing(dateStr);
  const currentPrice = checkoutUrl.match(/&price=(.*?)(?=&)/)[0].trim();
  checkoutUrl = checkoutUrl.replace(currentPrice, `&price=${price}`);

  if (price == lessThan3WeeksPrice) {
    const rushedName = `&name=Rush%20Delivery%20Tailored%20Plan`;
    const currentName = checkoutUrl.match(/&name=(.*?)(?=&)/)[0].trim();
    checkoutUrl = checkoutUrl.replace(currentName, rushedName);
  }

  return checkoutUrl;
}

function updateTravelDatesOnCheckoutUrl(selectedDates, checkoutUrl) {
  ...
  checkoutUrl = checkoutUrl.replace(/\&travel_dates=(.*?)((?=\&)|)$/g,'');
  checkoutUrl = `${checkoutUrl}&travel_dates=${travelDates}`;
...
/*function validateInputFields() {
  ...
  for (const inp of requiredInputs) {
    ...
    if (inp.getAttribute('type') === 'email' && !isValidEmail(val)) {
      highlight(inp);
      return;
    }
  }
