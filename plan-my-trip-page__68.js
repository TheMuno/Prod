...
!async function setupDatepicker() {
  ...
  const fp = flatpickr($travelDate, {
    ...
    // 🎨 Only tag static rush/high-demand visuals here (no green default)
    onDayCreate: function (dObj, dStr, fp, dayElem) {
      ...
      if (diffDays >= rushOrderDays && unavailable?.reason === "High Demand") {
    ...
    async onClose(selectedDates, dateStr, instance) {
      ...
      if (isHighDemand) {
        ...
        showError(
          `Sorry our team is at full capacity for your arrival date: ${arrivalDate}. 
          To be notified when capacity is free, send an email to hello@askkhonsu.com with subject "notify" and we will contact you when our team is free.`
        );
        instance.clear();
        return;
      }
  ...
  async function repaintDays(fpInstance, minHrs = setMinHrs) {
    ...
    dayEls.forEach((dayEl) => {
      ...
      // ⚠️ High Demand only outside rush
      ...
      // 🟢 Available — paint only after data known
