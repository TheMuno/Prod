...
!async function setupDatepicker() {
  ...
  function attachSmartTooltip() {
    ...
    document.addEventListener('mouseover', e => {
      ...
      const offset = 8; // spacing between tooltip and day cell
      let top = rect.bottom + offset;
      let left = rect.left + rect.width / 2;
      let flip = false;

      // Calculate tooltip width dynamically
      tooltip.style.opacity = '0'; // hide temporarily to measure
      ...
      // If tooltip would go below the viewport, flip it upward
      ...
      tooltip.style.setProperty('--arrow-color', '#222');
      tooltip.style.setProperty('--flip', flip ? 'up' : 'down');
      tooltip.setAttribute('data-flip', flip ? 'up' : 'down');

      // Update arrow direction with inline CSS
      const arrowColor = '#222';
      if (flip) {
        tooltip.style.setProperty('border', 'none');
