...
async function getUnavailableDates(minHrs = setMinHrs) {
  ...
  for (const day of daysData) {
    ...
    if (!available || available.toLowerCase() !== 'true' || capacityNum < minHrs) {
      unavailable.push(new Date(date)); // store as Date object
    }
  }
...
// USAGE EXAMPLE:
// (async () => {
//   const data = await fetchAllSheetDataDebounced();
//   console.log('Structured data rows:', data.length);
//   console.log(data.slice(0,5)); // sample
// })();
...
function showModal({ title = '', text = '', icon = 'info', confirmText = 'OK', timer = null }) {
  Swal.fire({
    ...
    background: '#fff',
    color: '#333',
    confirmButtonColor: '#007BFF', // brand blue (customize)
    showClass: {
