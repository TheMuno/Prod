...
// Initialize Stripe
const stripe = Stripe("pk_live_51I46LfBPPSonNtdr23fEGKl5x4dCpanLigGk64icRAXDeqOOLflZ4HaRO75IYWWwahS6quw6Glg2Bpessq9G5oAq00zX4jWnS0");

// Constants for server-side logic
const PRICE_STANDARD = "price_1SW0DeBPPSonNtdrO5pxx47B"; // "price_1SncyNBibJetjBciqaALqtUY"; 
const PRICE_RUSH = "price_1Sg4QlBPPSonNtdrMBRy2x0O"; // "price_1SnczbBibJetjBciWol2GzEN"; 
...
async function startCheckout(userData) {
  let eventDate = localStorage['ak-event-date-stripe'];
  if (!eventDate) {
      const savedFlatpickrObj = JSON.parse(localStorage['ak-flatpickrDateObj'] || '{}');
      const savedTravelDays = JSON.parse(localStorage['ak-travel-days'] || '{}');
      if (Object.keys(savedFlatpickrObj).length) {
        eventDate = JSON.parse(localStorage['ak-flatpickrDateObj']).selectedDates[0]; 
      }
      else if (Object.keys(savedTravelDays).length) {
        eventDate = JSON.parse(localStorage['ak-travel-days']).dateStr.split(/\s/)[0];
      }
      else {
        showError("No Trip Starting Date", "Please select a valid starting date for your trip.");
        return;
      }
  }
  ...
  try {
      ...
      if (dataResp.error) {
          ...
          showError("Failed Checkout", "Checkout failed: " + dataResp.error);
          return;
      }
      ...
      if (dataResp.url) {
          ...
          showError("Failed Stripe Session", "Failed to create Stripe session.");
      }

  } catch (error) {
      console.error('An unhandled error occurred:', error);
      showError("Internal Checkout Error", "An internal error prevented the checkout process.");
  }
}
  ...
  const fp = flatpickr($travelDate, {
    ...
    async onClose(selectedDates, dateStr, instance) {
      ...
      if (isHighDemand) {
        ...
        showError('High Demand', `Sorry our team is at full capacity for your arrival date: ${arrivalDate}. To be notified when capacity is free, send an email to hello@askkhonsu.com with subject "notify" and we will contact you when our team is free.`);
        instance.clear();
        return;
      }
...
function gatherUserDetails(emailField) {
  ...
  const purchaseType = 'PlanMyTrip';

  return { phoneNumber, userEmail, userName, firstName, lastName, arrivalDate, departureDate, purchaseType };
}
...
async function checkForCapacityOnDatePickerClose(dateArr, minHrs = setMinHrs) {
  ...
  try {
    ...
    if (!msg) {
      showError("High Demand", "Our queue is currently full for your travel dates. Please contact us at hello@askkhonsu.com for more info.");
      return null;
    } else {
    ...
    closeLoading();
    showError('Try Again', 'Something went wrong while checking capacity. Please try again.');
