...
!async function setupDatepicker() {
  ...
  // Add loading guard to prevent flicker on initial open
  ...
  const fp = flatpickr($travelDate, {
    ...
    // 🎨 Paint each day as it’s created (fast, synchronous)
    onDayCreate: function (dObj, dStr, fp, dayElem) {
      ...
      if (
        diffDays >= rushOrderDays &&
        unavailable &&
        unavailable.reason === "High Demand"
      ) {
        dayElem.classList.add("fp-unavailable");
        dayElem.dataset.tooltip = "High Demand";
      }

      // NOTE: we intentionally DO NOT add .fp-available here.
      // That used to cause flicker. Styling for available days will be applied
      // in the async repaint (setupDateIndicators) but we hide the calendar
      // while that repaint runs to avoid flashes.
    },
    ...
    async onClose(selectedDates, dateStr, instance) {
      ...
      const isHighDemand =
        diffDays >= rushOrderDays && startDayData?.reason === "High Demand";

      if (isHighDemand) {
        let arrivalDate = new Date(selectedDates[0]).toDateString();
        arrivalDate = `${arrivalDate.substring(0, 3)},${arrivalDate.substring(
          3
        )}`;
        showError(
          `Sorry our team is at full capacity for your arrival date: ${arrivalDate}. To be notified when capacity is free, send an email to hello@askkhonsu.com with subject "notify" and we will contact you when our team is free.`
        );
        instance.clear();
        return;
      }
    ...
    async onReady(selectedDates, dateStr, instance) {
      // ensure we hide calendar while we perform the async repaint to avoid flash
      instance.calendarContainer.classList.add("fp-loading");
      await setupDateIndicators(instance);
      $travelDate.classList.remove("fp-loading");
      instance.calendarContainer.classList.remove("fp-loading");
      instance.calendarContainer.classList.add("fp-ready");
    },
    async onMonthChange(selectedDates, dateStr, instance) {
      // show loading while we update visual state for the new month
      instance.calendarContainer.classList.add("fp-loading");
  ...
  // 🧩 Color + Tooltip setup (keeps your original logic)
  async function setupDateIndicators(fpInstance, minHrs = setMinHrs) {
    // Add loading guard on the container (extra safety)
    const calendar = fpInstance.calendarContainer;
    if (calendar) calendar.classList.add("fp-loading");
    ...
    dayEls.forEach((dayEl) => {
      ...
      if (!dayData) {
        // no data available for this date — keep it neutral (do NOT paint green)
        return;
      }
      ...
      // Only mark available when we have dayData and its capacity meets the minHrs
      if (available && capacity >= minHrs) {
        dayEl.classList.add("fp-available");
      }
    });

    // remove loading and show calendar (ensures no flicker)
    if (calendar) {
      // wait next frame so styles apply smoothly
      await new Promise((r) => requestAnimationFrame(r));
      calendar.classList.remove("fp-loading");
      calendar.classList.add("fp-ready");
    }
