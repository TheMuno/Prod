...
!async function setupDatepicker() {
  ...
  const fp = flatpickr($travelDate, {
    ...
    // minDate: 'today',
    disable: [
      ...disabledDates, // dynamic unavailable dates
      function (date) {
        return (date.getDay() === 0 || date.getDay() === 6); // 🚫 Automatically disable all weekends
      }
    ],
    ...
    onDayCreate: function (dObj, dStr, fp, dayElem) {
      ...
      if (dayOfWeek === 0 || dayOfWeek === 6) {
        ...
        dayElem.dataset.tooltip = 'Weekend — unavailable'; // use data-tooltip instead of title
      } else if (unavailable) {
        dayElem.classList.add('booked-disabled');
        dayElem.dataset.tooltip = unavailable.reason; // use data-tooltip
      }
    },

    // onReady: attachSmartTooltip,
  ...
  async function setupDateIndicators(fpInstance, minHrs = setMinHrs) {
    ...
    dayEls.forEach(dayEl => {
      ...
      dayEl.classList.remove('fp-available', 'fp-unavailable', 'fp-weekend', 'no-select');
      dayEl.disabled = false; // ensure we control this manually

      const dayIndex = dayEl.dateObj.getDay();
      const isWeekend = dayIndex === 0 || dayIndex === 6;

      // 🟠 Weekends — block selection
      if (isWeekend) {
        dayEl.classList.add('fp-weekend', 'no-select');
      ...
      // 🔴 Low Capacity — block selection
      if (!available || capacity < minHrs) {
        dayEl.classList.add('fp-unavailable', 'no-select');
      ...
      // dayEl.dataset.tooltip = `${capacity} hrs available`;
    });

    // Attach tooltips once all days are marked
    attachSmartTooltip();

    // Prevent clicking / keyboard selection
    preventSelection(fpInstance);
  }
  ...
  function attachSmartTooltip() {
    ...
    Object.assign(tooltip.style, {
      ...
      whiteSpace: 'nowrap',
      pointerEvents: 'none',
      zIndex: '999999', // ensure above Flatpickr
      opacity: '0',
  ...
  /*function attachSmartTooltip() {
    ...
    Object.assign(tooltip.style, {
      ...
      whiteSpace: 'nowrap',
      pointerEvents: 'none',
      zIndex: '999999', // <-- force above Flatpickr calendar
      opacity: '0',
      transform: 'translate(-50%, 0)',
      transition: 'opacity 0.15s ease, transform 0.15s ease',
    });
  ...
  function preventSelection(fpInstance) {
    const calendar = fpInstance.calendarContainer;
    if (!calendar) return;

    calendar.addEventListener('mousedown', e => {
      const target = e.target.closest('.flatpickr-day.no-select');
      if (target) {
        e.preventDefault();
    ...
    // Optional: prevent keyboard navigation selection
    calendar.addEventListener('keydown', e => {
      if (e.key === 'Enter' || e.key === ' ') {
        const focused = calendar.querySelector('.flatpickr-day:focus');
        if (focused && focused.classList.contains('no-select')) {
          e.preventDefault();
        }
      }
    });
  ...
  function addTooltipStyles() {
    ...
    style.textContent = `
      ...
      /* 🔥 Custom tooltip using data-tooltip instead of title * /
...
async function getUnavailableDates(minHrs = setMinHrs) {
  ...
  for (const day of daysData) {
    ...
    // ❌ Mark date as unavailable if not available or low capacity
    if (!available || available.toLowerCase() !== 'true' || capacityNum < minHrs) {
      unavailable.push({
        date: dateObj,
        reason: capacityNum < minHrs ? 'Low capacity' : 'Fully booked',
      });
    }
  }
